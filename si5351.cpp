...
void Si5351::init(uint8_t xtal_load_c, uint32_t ref_osc_freq)
{
	...
	si5351_write(SI5351_CLK2_CTRL, 0x80);
	si5351_write(SI5351_CLK3_CTRL, 0x80);
	si5351_write(SI5351_CLK4_CTRL, 0x80);
	si5351_write(SI5351_CLK5_CTRL, 0x80);
	si5351_write(SI5351_CLK6_CTRL, 0x80);
	si5351_write(SI5351_CLK7_CTRL, 0x80);
	*/

	/*
	// Set so that only MS fanout is enabled
	set_clock_fanout(SI5351_FANOUT_CLKIN, 0);
	set_clock_fanout(SI5351_FANOUT_XO, 0);
	set_clock_fanout(SI5351_FANOUT_MS, 1);
	*/

	/*
	// Turn the clocks back on...
	si5351_write(SI5351_CLK0_CTRL, 0x0c);
	si5351_write(SI5351_CLK1_CTRL, 0x0c);
	si5351_write(SI5351_CLK2_CTRL, 0x0c);
	si5351_write(SI5351_CLK3_CTRL, 0x0c);
	si5351_write(SI5351_CLK4_CTRL, 0x0c);
	si5351_write(SI5351_CLK5_CTRL, 0x0c);
	si5351_write(SI5351_CLK6_CTRL, 0x0c);
	si5351_write(SI5351_CLK7_CTRL, 0x0c);
	*/
...
uint8_t Si5351::set_freq(uint64_t freq, uint64_t pll_freq, enum si5351_clock clk)
{
	...
	// Set CLK source to MS
	set_clock_source(clk, SI5351_CLK_SRC_MS);
